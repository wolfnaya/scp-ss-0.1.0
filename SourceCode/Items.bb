...
;	it = CreateItemTemplate(GetLocalString("Item Names", "mag"), "extmag", "GFX\Weapons\Models\Attachments\extmag_box.b3d","GFX\items\Icons\Icon_misc.jpg","",0.02) : it\sound = 2 : it\fastPickable = True
...
;							EndIf
	...
	Else
		Select item\itemtemplate\tempname
			Case "suppressor","reddot","eotech","acog","match","mui","extmag_box"
				
				;CreateMsg(GetLocalString("Weapons","picked_suppressor"))
				PlaySound_Strict(item\itemtemplate\sound)
				...
				ElseIf item\itemtemplate\tempname = "mui" Then
					attint = ATT_MUI
				ElseIf item\itemtemplate\tempname = "extmag_box" Then
					attint = ATT_EXT_MAG
				EndIf
