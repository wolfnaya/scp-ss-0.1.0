...
Function FillRoom_Room1_Sewers(r.Rooms)
	...
	If gopt\GameMode = GAMEMODE_NTF Then
		fb\fuses = 2
		it = CreateItem(GetLocalString("Item Names","fuse"), "fuse", r\x - 776.0 * RoomScale, r\y - 5580.0 * RoomScale, r\z - 3728.0 * RoomScale)
	...
	r\Objects[ROOM1_SEWERS_ELEVATOR_ID] = LoadRMesh("GFX\map\elevators\elevator_cabin_2.rmesh", Null)
	...
	it = CreateItem(GetLocalString("Item Names","crowbar"), "crowbar", r\x - 128.0 * RoomScale, r\y - 5580.0 * RoomScale, r\z + 32.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	...
	it = CreateItem(GetLocalString("Item Names","broken_helmet"), "brokenhelmet", r\x - 4464.0 * RoomScale, r\y - 6206.0 * RoomScale, r\z - 6032.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	
	r\Objects[ROOM1_SEWERS_FAKEDOOR] = LoadRMesh("GFX\map\rooms\room1_sewers\room1_sewers_door.rmesh", Null)
	ScaleEntity r\Objects[ROOM1_SEWERS_FAKEDOOR],RoomScale,RoomScale,RoomScale
	EntityType(GetChild(r\Objects[ROOM1_SEWERS_FAKEDOOR], 2), HIT_MAP)
	EntityPickMode r\Objects[ROOM1_SEWERS_FAKEDOOR],2
	PositionEntity r\Objects[ROOM1_SEWERS_FAKEDOOR],r\x,r\y,r\z,True
	EntityParent r\Objects[ROOM1_SEWERS_FAKEDOOR],r\obj
	HideEntity r\Objects[ROOM1_SEWERS_FAKEDOOR]
...
Function UpdateEvent_Room1_Sewers(e.Events)
	...
	If gopt\GameMode = GAMEMODE_NTF And WasInPD Then
		If PlayerRoom = e\room Then
			
			If (Not EntityHidden(e\room\Objects[ROOM1_SEWERS_FAKEDOOR])) Then HideEntity(e\room\Objects[ROOM1_SEWERS_FAKEDOOR])
			...
			If EntityY(Collider)<-3500.0*RoomScale ;Sewers Area
				...
				PositionTexture e\room\Objects[ROOM1_SEWERS_WATERTEXTURE_ID], 0, e\EventState[2] * 0.002
			...
			If e\EventState[1] < 1.0 Then
				For fb = Each FuseBox
					If fb\obj = e\room\Objects[ROOM1_SEWERS_FUSEBOX_ID] Then
						If (Not TaskExists(TASK_NTF_FIND_FUSE)) And (Not TaskExists(TASK_NTF_FIND_FUSE_BOX)) And e\EventState[1] = 0.0 And m_msg\Txt = GetLocalString("Singleplayer", "fuse_find") Then
							BeginTask(TASK_NTF_FIND_FUSE)
							e\EventState[1] = 0.1
						EndIf	
						If fb\fuses = 3 Then
							EndTask(TASK_NTF_FIND_FUSE_BOX)
							If (Not Curr106\Contained) Then
								If (Not TaskExists(TASK_NTF_SEARCH_106)) Then
									BeginTask(TASK_NTF_SEARCH_106)
								EndIf
							EndIf
							...
							For i = 0 To MaxItemAmount-1
								If Inventory[i] <> Null Then
									If Inventory[i]\itemtemplate\tempname = "fuse" Then
										If (Not TaskExists(TASK_NTF_FIND_FUSE_BOX)) And e\EventState[1] = 0.0 Then
											BeginTask(TASK_NTF_FIND_FUSE_BOX)
										Else
											EndTask(TASK_NTF_FIND_FUSE)
										EndIf	
										Exit
									EndIf
								EndIf
			...
			Select e\EventState[0]
				...
				Case 1
					If EntityY(Camera) < EntityY(e\room\Objects[ROOM1_SEWERS_RAILINGPOINT], True) Then
						...
						If mpl\HasNTFGasmask Then
							For i = 0 To MaxItemAmount-1
								If Inventory[i] <> Null Then
									If Inventory[i]\itemtemplate\tempname = "ntf_helmet" Lor Inventory[i]\itemtemplate\tempname = "fine_ntf_helmet" Then
				...
				Case 3
					...
					EndIf
				Case 7
					If (Not TaskExists(TASK_NTF_FIND_WEAPON)) And (Not mtfd\IsPlaying) Then
						BeginTask(TASK_NTF_FIND_WEAPON)
					EndIf
					For i = 0 To MaxItemAmount-1
						If Inventory[i] <> Null Then
		...
		EndIf
	Else
		If EntityHidden(e\room\Objects[ROOM1_SEWERS_FAKEDOOR]) Then ShowEntity(e\room\Objects[ROOM1_SEWERS_FAKEDOOR])
	EndIf
